...
global ORDONNANCEMENT_SCIENCE = 1; //Spécial pour la science //Ne pas être un ordonnacement par defaut
global ORDONNANCEMENT_SUMMON_FIRST = 2;
global ORDONNANCEMENT_SUMMON_LAST = 3;
...
getActionFromCombo[ORDONNANCEMENT_SUMMON_FIRST] = function(@combo) {
	for (var i = 0; i < count(combo); i++) {
		if (combo[i][EFFECT] == EFFECT_SUMMON) {
			return @combo[i];
			//TODO: faire une règle de priorité entre plusieur summon ?
		}
	}
	...
	if (attack != [] && attack != null) {
		...
		if (attack[CALLBACK] !== null) {
			if (attack[PARAM] !== null) {
				...
				debug("callBack avec param");
			} else {
				attack[CALLBACK]();
				debug("callBack sans paramètre");
			}

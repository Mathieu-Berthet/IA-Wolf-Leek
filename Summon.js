...
function getSummonAction(@actions, @cellsAccessible, TPmax) {
	...
	for (var chip in chips) {
		if (isChip(chip) && getCooldown(chip) == 0 && TPmax >= getChipCost(chip) && (bulbeOffensif[chip] !== null or bulbeDefensif[chip] !== null or chip === CHIP_RESURRECTION)) {
			var tir;
			...
			debug(getChipName(chip) + " : " + tir[VALEUR]);
		}
	}
}

function resu() {
  var allieDead = getDeadAllies();
  if (allieDead == null || !count(allieDead)) return null;
  var allieToResurrect = allieDead[0];
  for(var allie = 1; allie < count(allieDead); allie++) {
    if(getTotalLife(allieDead[allie]) > getTotalLife(allieToResurrect) && !isSummon(allie)) {
...
function summonBulb(CHIP, IA, ennemie, @cellsAccessible) {
  ...
  tir[CALLBACK] = function (param) {//param = [chip, IA, cellsAccessible]
    ...
    if(getTP()-tir[PT_USE] <= 4) { // alors on se cache
      var tab = [];
      for (var cell : var dist in param[2]) push(tab, cell);
    ...
    var cellOuSummon = allCells[0];//pop(allCells); //TODO: Choisir correctement la cell où summon => la plus loin de l'adversaire ?

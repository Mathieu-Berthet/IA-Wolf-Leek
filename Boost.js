...
function getBoostAction(@actions, @cellsAccessible, Allies, TPmax) {
	var nb_action = count(actions); 
	var chips = getChips();
	for(var chip in chips) {
		if (getCooldown(chip) == 0 && TPmax >= getChipCost(chip)) {
			var effect = getChipEffects(chip);
			if(effect[0][0]== EFFECT_BUFF_AGILITY || effect[0][0] == EFFECT_BUFF_MP || effect[0][0] == EFFECT_BUFF_STRENGTH || effect[0][0] == EFFECT_BUFF_TP || effect[0][0] == EFFECT_BUFF_RESISTANCE || effect[0][0] == EFFECT_BUFF_WISDOM) {
...
function Booster(tool, allies, @cellsAccessible) {// pour les puces de soins sans AOE.   De boost*
		  	...
		  	if(!(MIN_RANGE[tool] != 0 && allie == ME)) {
				if(!haveEffect(allie,tool)) {
					...
					if (cell_deplace != -2) { //la cellule doit être atteignable
						...
						if(coeff===null) debugE("["+getChipName(tool)+"]Pas de valeur pour : "+ eff[TYPE]);
						valeur = coeff*(boost);

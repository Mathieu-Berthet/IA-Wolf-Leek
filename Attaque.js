// dernière mise à jour le : 02/02/2018 par : Caneton
//include("GLOBALS");
//include("getArea");
...
function getAttackAction(@actions, @cellsAccessible, toutEnnemis, TPmax) {
	...
	for (var i in chips) {
		...
		if (effet[0][0] == EFFECT_DAMAGE || effet[0][0] == EFFECT_POISON || effet[0][0] == EFFECT_SHACKLE_TP || effet[0][0] == EFFECT_SHACKLE_MP || effet[0][0] == EFFECT_SHACKLE_STRENGTH || effet[0][0] == EFFECT_SHACKLE_MAGIC)  {
			tab_damage[c] = i;
			c++;
		}
	}
	// Calcul
	for (var i in tab_damage) {
		var tir = [];
